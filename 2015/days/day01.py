...
Santa was hoping for a white Christmas, but his weather machine'inputs "snow" function is powered by stars, and he'inputs fresh
...
Here'inputs an easy puzzle to warm you up.
...
the basement (floor -1). The first character in the instructions has position 1, the second character has position 2,
and so on.
...
def part1(inputs):
    inputs = inputs.replace('(', '1 ').replace(')', '-1 ')
    inputs = [int(s) for s in inputs.strip().split(' ')]
    return sum(inputs)
...
puzzle_input = puzzle_input(1, 2015)
