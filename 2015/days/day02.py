...
face. Each present also requires a bow made out of ribbon as well; the feet of ribbon required for the perfect bow is equal to the cubic feet of volume of the present. Don't ask how they tie the bow, though; they'll never tell.
...
def part1(inputs):
    ...
    for i in inputs:
        i = [int(x) for x in i]
        a, b, c = 2 * i[0] * i[1], 2 * i[1] * i[2], 2 * i[0] * i[2]
        wrapping_paper += a + b + c + min(a, b, c)//2
...
def part2(inputs):
    ...
    for i in inputs:
        i = [int(x) for x in i]
        a, b, c = 2 * (i[0] + i[1]), 2 * (i[1] + i[2]), 2 * (i[0] + i[2])
        ribbon += min(a, b, c) + (i[0] * i[1] * i[2])

    return ribbon


puzzle_input = [x.split('x') for x in puzzle_input(2, 2015).split('\n')]
