...
hash is some secret key (your puzzle input, given below) followed by a number in decimal. To mine AdventCoins, you must find Santa the lowest positive number (no leading zeroes: 1, 2, 3, ...) that produces such a hash.
...
def part1(inputs):
    coins = 0
    while not hashlib.md5((inputs+str(coins)).encode('utf-8')).hexdigest().startswith('00000'):
        coins += 1
...
def part2(inputs):
    coins = 0
    while not hashlib.md5((inputs + str(coins)).encode('utf-8')).hexdigest().startswith('000000'):
        coins += 1

    return coins


puzzle_input = puzzle_input(4, 2015)
