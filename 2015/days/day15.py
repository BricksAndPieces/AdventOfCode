...
inputs = puzzle_input(15, 2015).split('\n')
ingredients = [[int(s) for s in re.findall('-?\\d+', i)] for i in inputs]
...
for amount in amounts:
    ...
    for p in range(len(ingredients[0]) - 1):
        value = sum([amount[i] * item[p] for i, item in enumerate(ingredients)])
        score *= value if value > 0 else 0

    ans1 = max(ans1, score)
    if sum([amount[i] * item[4] for i, item in enumerate(ingredients)]) == 500:
        ans2 = max(ans2, score)
