...
inputs = puzzle_input(19, 2015).split('\n\n')
starter_molecule = inputs[1]
...
for line in inputs[0].split('\n'):
    ...
    for i, j in enumerate(re.finditer(parts[0], starter_molecule)):
        end_molecule = starter_molecule[:j.start()] + parts[2] + starter_molecule[j.end():]
        molecules.add(end_molecule)

print(f'Part 1: {len(molecules)}')

counter = 0
while starter_molecule != 'e':
    counter += 1
    for key in replacements:
        if key in starter_molecule:
            index = starter_molecule.index(key)
            starter_molecule = starter_molecule[:index] + replacements[key] + starter_molecule[index + len(key):]
            break
    else:
        print('shit')
