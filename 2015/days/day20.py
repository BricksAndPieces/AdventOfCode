...
# inputs = int(puzzle_input(20, 2015))
# house = 831600
# while sum([i*10 for i in range(1, house+1) if not (house % i)]) < inputs:
#     house += 1


# 831600 correct
# Solution works but its takes waaaaaaaay to long
# Had to install numpy to gain a massive speed boost

inputs = int(puzzle_input(20, 2015))
houses_a = np.zeros(1_000_000)
houses_b = np.zeros(1_000_000)

for i in range(1, 1_000_000):
    houses_a[i::i] += i * 10
    houses_b[i:(i+1)*50:i] += i * 11


print(f'Part 1: {next(i for i, x in enumerate(houses_a) if x >= inputs)}')
print(f'Part 2: {next(i for i, x in enumerate(houses_b) if x >= inputs)}')
