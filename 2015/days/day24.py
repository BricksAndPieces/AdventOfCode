...
inputs = ints(puzzle_input(24, 2015))


def group(weight, packages=inputs):
    group = []
    for i in range(len(inputs) // 3):
        group += [j for j in combos(packages, i) if sum(j) == weight]

    group.sort(key=lambda x: reduce(lambda y, z: y * z, x))
    group.sort(key=lambda x: len(x))
    return group


weight_goal = sum(inputs) // 3
possible_group_1 = group(weight_goal)
for c in possible_group_1:
    remaining = [i for i in inputs if i not in c]
    if any([[[x == weight_goal for x in y] for y in combos(remaining, z)] for z in range(len(remaining) // 2)]):
        print(f'Part 1: {reduce(lambda x, y: x*y, c)}')
        break

# Part 2
weight_goal = sum(inputs) // 4
possible_group_1 = group(weight_goal)
for c in possible_group_1:
    remaining = [i for i in inputs if i not in c]
    possible_group_2 = group(weight_goal, remaining)
    for p in possible_group_2:
        remaining_2 = [i for i in remaining if i not in p]
        if any([[[x == weight_goal for x in y] for y in combos(remaining_2, z)] for z in range(len(remaining_2) // 2)]):
            print(f'Part 2: {reduce(lambda x, y: x * y, c)}')
            sys.exit(0)
