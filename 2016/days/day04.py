...
inputs = puzzle_input(4, 2016).split('\n')
ans1 = 0
ans2 = 0
...
for i in inputs:
    ...
    best = sorted(sorted(o), reverse=True, key=lambda x: o[x])
    checksum = i[-6:-1]

    if ''.join(best[:5]) == checksum:
        sector = int(i[-10:-7])
        ans1 += sector

        phrase = list(letters)
        for x in range(sector % 26):
            for index, letter in enumerate(phrase):
                phrase[index] = chr(ord(letter) + 1) if letter != 'z' else 'a'

        if ''.join(phrase) == 'northpoleobjectstorage':
            ans2 = sector
