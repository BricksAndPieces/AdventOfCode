...
angrily about a "1202 program alarm". On the radio, an Elf is already explaining how to handle the situation: "Don't
worry, that'inputs perfectly norma--" The ship computer bursts into flames.

You notify the Elves that the computer'inputs magic smoke seems to have escaped. "That computer ran Intcode programs like
...
1,9,10,3, 2,3,11,0, 99, 30,40,50 The first four integers, 1,9,10,3, are at positions 0, 1, 2, and 3. Together,
they represent the first opcode (1, addition), the positions of the two inputs (9 and 10), and the position of the
output (3). To handle this opcode, you first need to get the values at the input positions: position 9 contains 30,
...
1,9,10,70, 2,3,11,0, 99, 30,40,50 Step forward 4 positions to reach the next opcode, 2. This opcode works just like
the previous, but it multiplies instead of adding. The inputs are at positions 3 and 11; these positions contain 70
...
1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2). 2,3,0,3,99 becomes 2,3,0,6,99 (3 * 2 = 6). 2,4,4,5,99,0 becomes 2,4,4,5,
99,9801 (99 * 99 = 9801). 1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99. Once you have a working computer,
the first step is to restore the gravity assist program (your puzzle input) to the "1202 program alarm" state it had
just before the last computer caught fire. To do this, before running the program, replace position 1 with the value
12 and replace position 2 with the value 2. What value is left at position 0 after the program halts?
...
Intcode programs are given as a list of integers; these values are used as the initial state for the computer'inputs
memory. When you run an Intcode program, make sure to start by initializing memory to the program'inputs values. A
...
are called the instruction'inputs parameters. For example, in the instruction 1,2,3,4, 1 is the opcode; 2, 3,
...
inputs, make sure you first reset the computer'inputs memory to the values in the program (your puzzle input) - in other
...
def part1(inputs):
    ...
    inputs[1] = 12
    inputs[2] = 2
    return int_code(inputs)[0]


def part2(inputs):
    for noun in range(100):
        for verb in range(100):
            codesCopy = inputs.copy()
            codesCopy[1] = noun
            codesCopy[2] = verb

            if int_code(codesCopy)[0] == 19690720:
                return 100 * noun + verb


puzzle_input = ints(puzzle_input(2, '2019'), ',')
