...
refuelling station. During the rush back on Earth, the fuel management system wasn't completely installed,
so that'inputs next on the priority list.
...
For example, if the first wire'inputs path is R8,U5,L5,D3, then starting from the central port (o), it goes right 8, up 5,
...
.o-------+.
...........
Then, if the second wire'inputs path is U7,R6,D4,L4, it goes up 7, right 6, down 4, and left 4:
...
def steps_for_coords(code, coords):
    x, y = 0, 0
    steps = 0
    for s in code:
        for _ in range(int(s[1:])):
...
def part1(inputs):
    intersects = set(get_coords(inputs[0])).intersection(get_coords(inputs[1]))
    return min([abs(x[0]) + abs(x[1]) for x in intersects])


def part2(inputs):
    coords1 = get_coords(inputs[0])
    coords2 = get_coords(inputs[1])

    intersects = set(coords1).intersection(coords2)
    steps = [(steps_for_coords(inputs[0], x) + steps_for_coords(inputs[1], x)) for x in intersects]
    return min(steps)


puzzle_input = [s.split(',') for s in puzzle_input(3, 2019).split('\n')]
