...
def part1(inputs):
    ...
    for i in inputs:
        ...
        parent = i[0]
        while parent != 'COM':
            parent = [x[0] for x in inputs if x[1] == parent][0]
            orbits += 1
...
def part2(inputs):
    san_to_com = []
    parent = [x[0] for x in inputs if x[1] == 'SAN'][0]
    while parent != 'COM':
        san_to_com.append(parent)
        parent = [x[0] for x in inputs if x[1] == parent][0]

    you_to_com = []
    parent = [x[0] for x in inputs if x[1] == 'YOU'][0]
    while parent != 'COM':
        you_to_com.append(parent)
        parent = [x[0] for x in inputs if x[1] == parent][0]

    duplicates = 0
    for i in range(len(san_to_com)):
        if san_to_com[-i-1] == you_to_com[-i-1]:
            duplicates += 1
        else:
            break

    return len(san_to_com) + len(you_to_com) - duplicates * 2


puzzle_input = [x.split(')') for x in puzzle_input(6, 2019).split('\n')]
