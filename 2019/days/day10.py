...
def asteroids_detected(pos, space):
    angles = set()
    for x in range(len(space)):
        for y in range(len(space[0])):
            if (x, y) != pos and space[x][y] == '#':
                angles.add(math.atan2(pos[1]-y, pos[0]-x))

    return len(angles)


def death_laser(pos, space):
    angles = {}
    for x in range(len(space)):
        for y in range(len(space[0])):
            if (x, y) != pos and space[x][y] == '#':
                k = math.degrees(math.atan2(pos[1] - y, pos[0] - x))
                k %= 360

                if k not in angles or abs(pos[0]-x) < abs(pos[0]-angles[k][0]):
                    angles[k] = x, y

    return angles


inputs = puzzle_input(10, 2019).split('\n')
most = 0
pos = -1
for x in range(len(inputs)):
    for y in range(len(inputs[0])):
        if inputs[x][y] == '#':
            count = asteroids_detected((x, y), inputs)
            if count > most:
                most = count
                pos = x, y
...
angles = death_laser(pos, inputs)
keys = sorted(angles)
key = keys[len(keys) - 199]
pos = angles[key]

print(f'Part 2: {pos[1] * 100 + pos[0]}')
