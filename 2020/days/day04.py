...
def part2(inputs):
    ...
    for i in inputs:
        ...
        for z in i:
            if z.startswith('byr'):
                year = int(z[-4:])
                if (not len(z[z.index(':') + 1:]) == 4) or year < 1920 or year > 2002:
                    x = False
                    break
            elif z.startswith('iyr'):
                year = int(z[-4:])
                if (not len(z[z.index(':') + 1:]) == 4) or year < 2010 or year > 2020:
                    x = False
                    break
            elif z.startswith('eyr'):
                year = int(z[-4:])
                if (not len(z[z.index(':') + 1:]) == 4) or year < 2020 or year > 2030:
                    x = False
                    break
            elif z.startswith('hgt'):
                h = int(z.replace('hgt:', '').replace('cm', '').replace('in', ''))
                if not ('in' in z or 'cm' in z):
                    x = False
                    break
                if 'cm' in z:
                    if h < 150 or h > 193:
                ...
                g = ['a', 'b', 'c', 'c', 'd', 'e', 'f']
                for o in h[1:]:
                    if not (o.isdigit() or o in g):
                        print(o)
                        x = False
                        break
                if not x:
                    break
            elif z.startswith('ecl'):
                g = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
                h = z[4:]
                if h not in g:
                    x = False
                    break
            elif z.startswith('pid'):
                h = z[4:]
                if len(h) != 9:
                    x = False
                    break
...
puzzle_input = puzzle_input(4, 2020).split('\n')
puzzle_input = str(puzzle_input).replace('\'\'', '.').replace('[', '').replace(']', '')
