...
inputs = [list(i) for i in puzzle_input(11, 2020).split('\n')]
...
def num_adjacent(x, y, s, part):
    ...
    for dx, dy in product([-1, 0, 1], repeat=2):
        ...
        if part == 1:
            if 0 <= x + dx < len(s) and 0 <= y + dy < len(s[0]) and s[x + dx][y + dy] == '#':
                count += 1
        elif part == 2:
            for i in range(1, len(s)):
                if not (0 <= x + dx * i < len(s) and 0 <= y + dy * i < len(s[0])) or s[x + dx * i][y + dy * i] == 'L':
                    break
...
def stable_occupied_seats(part):
    ...
    while True:
        ...
        for x in range(len(new)):
            for y in range(len(new[0])):
                if current[x][y] == 'L' and num_adjacent(x, y, current, part) == 0:
                    new[x][y] = '#'
                elif current[x][y] == '#' and num_adjacent(x, y, current, part) >= (5 if part-1 else 4):
                    new[x][y] = 'L'
...
print(f'Part 1: {stable_occupied_seats(1)}')
print(f'Part 2: {stable_occupied_seats(2)}')
