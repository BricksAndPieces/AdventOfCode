...
inputs = puzzle_input(14, 2020).split('\n')
memory1 = {}
memory2 = {}
...
while line < len(inputs):
    ...
    while line < len(inputs) and inputs[line].startswith('mem'):
        mem = inputs[line]
        pointer = int(mem[mem.index('[')+1: mem.index(']')])
        value = decimalToBinary(int(mem.split(' = ')[1]))

        # Part 1
        result = [value[i] if mask_val == 'X' else mask_val for i, mask_val in enumerate(mask)]
        memory1[pointer] = ''.join(result)
        line += 1

        # Part 2
        original_pointer = decimalToBinary(pointer)
        x_values = list(product([0, 1], repeat=mask.count('X')))
        for address_count in range(len(x_values)):
            pointer = []
            x_count = 0
            for i, mask_val in enumerate(mask):
                if mask_val == '0':
                    pointer.append(original_pointer[i])
                elif mask_val == '1':
                    pointer.append('1')
                else:
                    pointer.append(str(x_values[address_count][x_count]))
                    x_count += 1

            memory2[binary_2_decimal(''.join(pointer))] = value


print(f'Part 1: {sum([binary_2_decimal(x) for x in memory1.values()])}')
print(f'Part 2: {sum([binary_2_decimal(x) for x in memory2.values()])}')
