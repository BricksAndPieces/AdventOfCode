...
Turn 6: The last number spoken, 3 had also been spoken before, most recently on turns 5 and 2. So, the 6th number spoken
is 5 - 2, 3.
...
def solve(num):
    ...
    for i in range(num):
        if i < len(inputs):
            numbers[inputs[i]] = deque(maxlen=2)
            numbers[inputs[i]].append(i)
            last = inputs[i]
            continue

        if last in numbers and len(numbers[last]) > 1:
            key = i - numbers[last][-2] - 1
            if key not in numbers:
                numbers[key] = deque(maxlen=2)

            numbers[key].append(i)
            last = key
        else:
            if 0 not in numbers:
                numbers[0] = deque(maxlen=2)
...
inputs = ints(puzzle_input(15, 2020), ',')
