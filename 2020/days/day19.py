...
inputs = puzzle_input(19, 2020, sample=False).split('\n\n')
rules_raw = inputs[0].split('\n')
msgs = inputs[1].split('\n')
...
for r in rules_raw:
    k = int(r[:r.index(':')])
    if '"' in r:
        rules[k] = r[r.index(':') + 2:].replace('"', '')
        continue

    r = r[r.index(':') + 2:].split(' | ')
    rules[k] = [[int(x) for x in z.split(' ')] for z in r]


def match(msg, rule=0, index=0):
    if index == len(msg):
        return set()

    if isinstance(rules[rule], str):
        return {index+1} if rules[rule] == msg[index] else set()

    matches = set()
    for sub in rules[rule]:
        indexes = {index}
        for r in sub:
            indexes = reduce(lambda x, y: x | y, [match(msg, r, i) for i in indexes])
            if not indexes:
                break
        else:
            matches |= indexes
...
print(f'Part 1: {sum(len(m) in match(m) for m in msgs)}')

rules[8] = [[42], [42, 8]]
rules[11] = [[42, 31], [42, 11, 31]]
print(f'Part 2: {sum(len(m) in match(m) for m in msgs)}')
