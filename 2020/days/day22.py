...
Otherwise, at least one player must not have enough cards left in their deck to recurse; the winner of the round is the
...
inputs = puzzle_input(22, 2020, sample=False).split('\n\n')
player1 = [int(x) for x in inputs[0].split('\n')[1:]]
player2 = [int(x) for x in inputs[1].split('\n')[1:]]


def combat(p1, p2, part):
    previous_states = set()
    while p1 and p2:
        state = (tuple(p1), tuple(p2))
        if part == 2 and state in previous_states:
            return True, p1

        previous_states.add(state)
        a, b = p1.pop(0), p2.pop(0)

        recursive = part == 2 and a <= len(p1) and b <= len(p2)
        p1_wins = combat(p1.copy()[:a], p2.copy()[:b], part)[0] if recursive else a > b
        winner = p1 if p1_wins else p2
        winner += [a, b] if p1_wins else [b, a]

    return p1, p1 if p1 else p2


for part in 1, 2:
    winner = combat(player1.copy(), player2.copy(), part)[1]
