...
inputs = puzzle_input(24, 2020, sample=False).split('\n')

dirs = ['nw', 'se', 'ne', 'sw', 'w', 'e']
dirs_replace = list('ABCDEF')

black = set()
for line in inputs:
    for i, dir in enumerate(dirs):
        line = line.replace(dir, dirs_replace[i])
...
for _ in range(100):
    ...
    test = black.copy()
    for x, y, z in black:
        for dx, dy, dz in permutations([-1, 0, 1]):
            test.add((x+dx, y+dy, z+dz))

    for x, y, z in test:
        tile = (x, y, z) in black
        n = len([0 for dx, dy, dz in permutations([-1, 0, 1]) if (x+dx, y+dy, z+dz) in black])
        if (tile and n in [1, 2]) or (not tile and n == 2):
            new_black.add((x, y, z))
