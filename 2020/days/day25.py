...
Now, the card has the door's public key, and the door has the card's public key. Because you can eavesdrop on the
...
inputs = puzzle_input(25, 2020, sample=False).split('\n')
loop = get_loop_size(7, int(inputs[0]))
key = transform(int(inputs[1]), loop)
